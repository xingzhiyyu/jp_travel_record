...
明确区分 JR 宇治/京阪宇治、江ノ島站/江之岛、伏见稻荷站/稻荷站。etc
只补有唯一上下文解释的起終点。歧义问用户，独立可完成部分继续做。
中文名称映射优先使用生成器日文名称和项目别名；缺失站点核对运营商、地区、经纬度。
铁路端点优先选择 station/halt 等交通对象；同名景点、街区或岛屿不能作为车站候选。
方向后缀（如 `Osaka-bound`、`方面`、`行き`）不属于线路身份，清洗时保留其语义，
解析时必须锁定已知 canonical line，不能因此回退到相似名称的其他运营商线路。
...
每个底图覆盖区及其边界前后至少检查一组连续帧；乡郊镜头检查 secondary/tertiary 道路、
溪流等地理参照。巨大直线或三角色块须追踪到具体 OSM 对象和来源，低精度规划面不能用于近景。
官方线路色同时检查日间和夜间，不能只检查解析清单中的十六进制值。
...
`--resolve-only` 不会预热渲染时懒加载的环境、道路和背景轨道。离线整片前先用实际渲染器抽取
沿途各区域和边界的代表帧，确认清单中没有 `unavailable; omitted`、道路单元缺失或非预期直线回退；
否则停止正式渲染，联网补齐后再复核。
