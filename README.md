...
- 但对于user来讲,可以直接用任何形式的方式输入,让AI自己清理数据。但不要把JR的机器打印IC卡履历拍照交给AI，这个表只有起终点，没法判断具体路径，需要回忆然后喂给AI。
- 生成器可以生成一些简单的符合条件的线路，包括步行驾车和几乎所有地铁、火车、电车，也可以人工写路程，人工的路程可以直接交给AI处理。
- 经测试Workbuddy的免费Hy4-preview就可以完成生成视频的任务，可以把项目对应的链接发给workbuddy，或其他agent，也可以在页面里download zip，在文件夹里解压，再在文件夹里启动agent。
- 渲染时间会很长，建议连接电源，同时定时检查避免电脑息屏
- AI 助手交接：[执行流程](docs/WORKFLOW.md)、[可复制提示词](docs/ASSISTANT_PROMPT.md)。
- 准备上传仓库：[GitHub 文件清单](docs/GITHUB_UPLOAD.md)。助手入口为根目录 AGENTS.md。
- 个人行程、地图缓存与视频默认不进入版本控制,也请不要往仓库上传你的行程。
...
- 新干线缩放分类支持中日英文、简繁体、假名及常见列车名（含车次号、Max / Super 变体），并参考已解析的 OSM 名称与规范线路名。它们不再误用普通长距离铁路的 `12.5` 近景下限；换乘前后各一秒的过渡保持不变。具体范围和歧义见 [新干线识别](docs/SHINKANSEN.md)。这只是镜头分类，不表示所有列车名已能直接联网解析，也不会替换用户输入或编造线路关系。
...
- 可设置 `TRAVEL_RECORD_OFFLINE=1` 仅使用已有缓存。离线时若沿途环境或矢量道路缓存缺失，silhouette 渲染会报错，不再静默生成空白底图。视频成功编码后才替换目标文件，中断时保留上一次完整成品。
